...
class APIClient {
    ...
    func request<T: Codable>(_ path: String, method: String = "GET", body: (any Encodable)? = nil, retryCount: Int = 0) async throws -> T {
        ...
        req.setValue("RouaTrading/3.0 (iOS; Mobile)", forHTTPHeaderField: "User-Agent")
        ...
        if let token = sessionToken {
            ...
            print("[API] → \(method) \(path) [NO AUTH TOKEN — proxy will auto-create session]")
        }
        ...
        guard (200...299).contains(http.statusCode) else {
            ...
            print("[API] ❌ Error \(http.statusCode) on \(method) \(path): \(errorBody.prefix(500))")
